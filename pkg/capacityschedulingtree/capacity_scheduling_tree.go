...
package capacityschedulingtree
...
// CapacitySchedulingTree is a plugin that implements the mechanism of capacity scheduling.
type CapacitySchedulingTree struct {
	sync.RWMutex
	fh                 framework.Handle
	podLister          corelisters.PodLister
	pdbLister          policylisters.PodDisruptionBudgetLister
	elasticQuotaLister externalv1alpha1.ElasticQuotaTreeLister
	elasticQuotaInfos  ElasticQuotaTreeInfos
}
...
// ElasticQuotaTreeSnapshotState stores the snapshot of elasticQuotas.
type ElasticQuotaTreeSnapshotState struct {
	elasticQuotaInfos ElasticQuotaTreeInfos
}

// Clone the ElasticQuotaSnapshot state.
func (s *ElasticQuotaTreeSnapshotState) Clone() framework.StateData {
	return &ElasticQuotaTreeSnapshotState{
		elasticQuotaInfos: s.elasticQuotaInfos.clone(),
	}
}

var _ framework.PreFilterPlugin = &CapacitySchedulingTree{}
var _ framework.PostFilterPlugin = &CapacitySchedulingTree{}
var _ framework.ReservePlugin = &CapacitySchedulingTree{}

var _ framework.EnqueueExtensions = &CapacitySchedulingTree{} // for job queue
var _ preemption.Interface = &preemptor{}

const (
	// Name is the name of the plugin used in Registry and configurations.
	Name = "CapacitySchedulingTree"

	// preFilterStateKey is the key in CycleState to NodeResourcesFit pre-computed data.
	preFilterStateKey       = "PreFilter" + Name
	ElasticQuotaSnapshotKey = "ElasticQuotaTreeSnapshot"
)

// Name returns name of the plugin. It is used in logs, etc.
func (c *CapacitySchedulingTree) Name() string {
	return Name
}

// New initializes a new plugin and returns it.
func New(obj runtime.Object, handle framework.Handle) (framework.Plugin, error) {
	c := &CapacitySchedulingTree{
		fh:                handle,
	...
	klog.InfoS("CapacitySchedulingTree start")
	return c, nil
}

func (c *CapacitySchedulingTree) EventsToRegister() []framework.ClusterEvent {
...
func (c *CapacitySchedulingTree) PreFilter(ctx context.Context, state *framework.CycleState, pod *v1.Pod) *framework.Status {
...
func (c *CapacitySchedulingTree) PreFilterExtensions() framework.PreFilterExtensions {
	return c
}

// AddPod from pre-computed data in cycleState.
func (c *CapacitySchedulingTree) AddPod(ctx context.Context, cycleState *framework.CycleState, podToSchedule *v1.Pod, podToAdd *framework.PodInfo, nodeInfo *framework.NodeInfo) *framework.Status {
	elasticQuotaSnapshotState, err := getElasticQuotaTreeSnapshotState(cycleState)
	if err != nil {
...
func (c *CapacitySchedulingTree) RemovePod(ctx context.Context, cycleState *framework.CycleState, podToSchedule *v1.Pod, podToRemove *framework.PodInfo, nodeInfo *framework.NodeInfo) *framework.Status {
	elasticQuotaSnapshotState, err := getElasticQuotaTreeSnapshotState(cycleState)
	if err != nil {
		klog.ErrorS(err, "Failed to read elasticQuotaSnapshot from cycleState", "elasticQuotaSnapshotKey", ElasticQuotaSnapshotKey)
		return framework.NewStatus(framework.Error, err.Error())
...
func (c *CapacitySchedulingTree) PostFilter(ctx context.Context, state *framework.CycleState, pod *v1.Pod, m framework.NodeToStatusMap) (*framework.PostFilterResult, *framework.Status) {
	defer func() {
...
func (c *CapacitySchedulingTree) Reserve(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) *framework.Status {
	c.Lock()
	defer c.Unlock()
...
func (c *CapacitySchedulingTree) Unreserve(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) {
	c.Lock()
	defer c.Unlock()
...
func (p *preemptor) PodEligibleToPreemptOthers(pod *v1.Pod, nominatedNodeStatus *framework.Status) bool {
	...
	if len(nomNodeName) > 0 {
		...
		elasticQuotaSnapshotState, err := getElasticQuotaTreeSnapshotState(p.state)
		if err != nil {
			klog.ErrorS(err, "Failed to read elasticQuotaSnapshot from cycleState", "elasticQuotaSnapshotKey", ElasticQuotaSnapshotKey)
			return true
		}
...
func (p *preemptor) SelectVictimsOnNode(
	...
	// Get EQ State
	elasticQuotaSnapshotState, err := getElasticQuotaTreeSnapshotState(state)
	if err != nil {
...
func (c *CapacityScheduling) snapshotElasticQuota() *ElasticQuotaTreeSnapshotState {
	c.RLock()
	defer c.RUnlock()
...
// getElasticQuotaTreeSnapshotState obtains ElasticQuotaTreeSnapshotState from storage. The state consists of
// pairs of "namespace" and "quota"
func getElasticQuotaTreeSnapshotState(cycleState *framework.CycleState) (*ElasticQuotaTreeSnapshotState, error) {
	c, err := cycleState.Read(ElasticQuotaSnapshotKey)
	if err != nil {
		// ElasticQuotaTreeSnapshotState doesn't exist, likely PreFilter wasn't invoked.
		return nil, fmt.Errorf("error reading %q from cycleState: %v", ElasticQuotaSnapshotKey, err)
	}

	s, ok := c.(*ElasticQuotaTreeSnapshotState)
	if !ok {
		return nil, fmt.Errorf("%+v  convert to CapacityScheduling ElasticQuotaTreeSnapshotState error", c)
	}
	return s, nil
}
